...
for (j in 1:runs)
  {
      ...
      Nat.FryToPresmoltSurv <- vector(length=years)
        for(m in 1:years)
          {
          Nat.FryToPresmoltSurv[m] <- sample(Nat.FryToPresmoltSurv.list, 1, replace = TRUE, prob = NULL)
          }
      Hatch.FryToPresmoltSurv <- vector(length=years)
        for(m in 1:years)
          {
          Hatch.FryToPresmoltSurv[m] <- sample(Hatch.FryToPresmoltSurv.list, 1, replace = TRUE, prob = NULL)
          }
      ...
      for (i in 1:5)   #seeding the adults for years 1-5
      {
        ...
        N.new[5] = N.new[3]*(Nat.FryToPresmoltSurv[i]*stoch)     #natural-origin pre-smolt/smolts
          NatOrigPreSmolts.raw[i,j] <- N.new[5]
        N.new[6] = N.new[4]*(Hatch.FryToPresmoltSurv[i]*stoch)     #hatchery-origin pre-smolt/smolts
    ...
    for (i in 6)   #now can add up ocean age 1-3 adults to get adult back by their spawning year
      {
        ...
        # calculate spawners in hatchery vs. in river          
          #estimate PropToHatchery based on number of adults to the river 
          #scenario #7
          ...
          if ((N.new[9]+N.new[10])<5000) {
             ...
             PropToHatchery[i] <- 0.05*stoch
                
          #this is special, no fish to hatchery at >80000 spawners scenario--#6a
          #PropToHatchery[i] <- 0.05
          #if ((N.new[9]+N.new[10])<5000) {
          #      PropToHatchery[i] <- 0.5*stoch
          #      } else if ((N.new[9]+N.new[10])<15000) {
          ...
          #      } else if ((N.new[9]+N.new[10])<50000) {
          #      PropToHatchery[i] <- 0.1*stoch
          #      } else if ((N.new[9]+N.new[10])<80000) {
          #      PropToHatchery[i] <- 0.05*stoch
          #      } else  
          #      PropToHatchery[i] <- 0          
...
#write.csv(MedianSpawners,"C:\\Users\\kendanwk\\OneDrive - Washington State Executive Branch Agencies\\WDFW\\Cedar River sockeye\\Life-cycle model\\Output\\MedianSpawners.csv")
#write.csv(MedianpHOS,"C:\\Users\\kendanwk\\OneDrive - Washington State Executive Branch Agencies\\WDFW\\Cedar River sockeye\\Life-cycle model\\Output\\MedianpHOS.csv")
